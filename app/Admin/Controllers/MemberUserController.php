...
class MemberUserController extends AdminController
{
    ...
    protected function grid()
    {
        return Grid::make(new MemberUser, function (Grid $grid) {
            ...
            $grid->quickSearch(['username', 'phone', 'email'])->placeholder('请输入用户名、手机或邮箱');
            ...
            $grid->filter(function (Grid\Filter $filter) {
                $filter->panel();
                $filter->expand(false); // 默认展开过滤器
                $filter->like('username')->width(3)->placeholder('请输入用户名');
                $filter->like('phone')->width(3)->placeholder('请输入手机号');
                $filter->like('email')->width(3)->placeholder('请输入邮箱');

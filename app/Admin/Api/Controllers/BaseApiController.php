...
abstract class BaseApiController extends Controller
{
    ...
    /** @var array 字段名缓存，避免重复查询 */
    protected static array $fieldNameCache = [];
    ...
    public function downImportTplFile(Request $request)
    {
        ...
        // 获取所有字段及注释（兼容 MySQL/PostgreSQL/SQLite/SQL Server）
        ...
        foreach ($columns as $col) {
            ...
            } elseif (str_contains($type, 'char') || str_contains($type, 'text') || str_contains($type, 'varchar')) {
    ...
    protected function exportData(Request $request)
    {
        ...
        $sort['field'] = $request->input('sort_field', 'id');
        $sort['direction'] = $request->input('sort_order', 'asc');
    ...
    public function field(Request $request)
    {
        $table = $this->model->getTable();
        // 获取所有字段及注释（兼容所有数据库驱动）
        $columns = collect(\Schema::getColumns($table))->map(function ($col) {
            return [
                'name' => $col['name'],
                'type' => $col['type_name'] ?? ($col['type'] ?? ''),
                'nullable' => $col['nullable'] ?? true,
    ...
    public function getFullFieldName($exclude = ['id', 'created_at', 'updated_at', 'deleted_at'])
    {
        ...
        return static::$fieldNameCache[$cacheKey] = $columns;
    }
